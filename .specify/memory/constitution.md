...
O Postgres é um **índice derivado** desses arquivos: permite listar,
...
dado que não exista em arquivo ou que o sync não consiga reconstruir.

### II. Testes funcionais reais (NON-NEGOTIABLE)
Testes rodam contra **Postgres real** (`TEST_DATABASE_URL`) e **filesystem real**
(`CONTENT_ROOT` temporário). A única coisa mockada é a fronteira
framework/identidade; banco, disco e sanitizador nunca são mockados. Toda
funcionalidade ativada por código entrega junto o teste que a prova (ou, para
funções puras de risco, um check autônomo que falhe se a lógica quebrar).
...
O portal não depende de nenhum serviço externo para operar. Deploy é rede local
com comandos simples (`build` + `db:deploy` + `start`). Os dois ativos de estado
— banco (RBAC, metadados, histórico) e `content/departamentos/` — têm backup
coberto; a restauração deve ser testada periodicamente. O conjunto
container/serviço local nunca é pré-requisito: Docker é conveniência, não
dependência.
...
+ shadcn/ui, PostgreSQL 17, Prisma 7 (driver adapter `@prisma/adapter-pg`),
NextAuth v5 (Auth.js) Credentials + JWT, `isomorphic-dompurify`. **Node.js
22.19+** é requisito declarado no `engines`, não preferência. A connection
string vive em `prisma.config.ts`; o client é gerado em `src/generated/prisma/`
e não é versionado. O padrão de conteúdo é o descrito em `content/TEMPLATE.md`;
documentação que não participa do ciclo de revisão não ganha selo.
...
4. `npm test` — suíte funcional contra Postgres real (`npm run db:up` sobe um).
